const About = () => {
  return (
    <aside id="about" className="flex-col flex max-xmd:px-2 justify-between gap-4 pt-24">
      <p className="text-p-1">
        I am a dedicated{" "}
        <span className="text-light-1">Full Stack developer</span> with a focus
        on <span className="text-light-1">JavaScript</span> technologies, adept
        in both <span className="text-light-1">SQL</span> and{" "}
        <span className="text-light-1">NoSQL</span> databases. My journey in
        programming commenced on January 1, 2022, and since then, I've
        passionately pursued self-learning and continuous expansion of my skill
        set.
      </p>

      <p className="text-p-1">
        I thrive on embracing new challenges and opportunities for professional
        growth. My core emphasis lies in crafting efficient, scalable solutions
        that prioritize an exceptional user experience. I consider myself an
        ardent <span className="text-light-1">problem solver</span>, always
        eager to confront fresh challenges and acquire knowledge in emerging
        technologies.
      </p>
    </aside>
  );
};

export default About;
